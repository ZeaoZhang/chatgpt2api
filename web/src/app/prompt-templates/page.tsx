...
import { Download, LoaderCircle, Plus, RefreshCw, Search, Trash2 } from "lucide-react";
    ...
    <section className="space-y-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button onClick={() => setEditorOpen(true)} className="h-10 rounded-xl bg-stone-950 text-white hover:bg-stone-800">
      ...
      <Card className="rounded-2xl border-white/80 bg-white/90 shadow-sm">
        <CardContent className="p-0">
          ...
          <div className="divide-y divide-stone-100">
            {items.map((item) => (
              <article key={item.id} className="grid gap-3 px-5 py-4 transition hover:bg-stone-50 lg:grid-cols-[minmax(0,1fr)_auto]">
      ...
      <Dialog open={editorOpen} onOpenChange={setEditorOpen}>
        <DialogContent className="w-[min(92vw,720px)] rounded-2xl">
          ...
          <DialogFooter>
            <Button variant="outline" className="rounded-xl" onClick={() => setEditorOpen(false)} disabled={isSaving}>
              取消
            </Button>
            <Button className="rounded-xl bg-stone-950 text-white hover:bg-stone-800" onClick={() => void handleCreateTemplate()} disabled={isSaving}>
